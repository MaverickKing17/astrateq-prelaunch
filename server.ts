...
app.post("/api/send-reservation-email", async (req, res) => {
  try {
    ...
    const fromAddress = "Astrateq Reservations <reservations@astrateqgadgets.com>";
    const resolvedTimestamp = timestamp ? new Date(timestamp).toLocaleString("en-US", { timeZone: "America/Toronto" }) + " EST" : new Date().toLocaleString("en-US", { timeZone: "America/Toronto" }) + " EST";
    const amount = simulatedAmount || "$0.00 CAD";

    const subject = "Your Astrateq Founding Cohort Reservation Was Received";

    // Build highly-polished template with direct focus on requirements
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    console.log("Attempting to send Astrateq confirmation email");
...
// API Route - Simple Test Endpoint to verify Resend functionality independently of Reservation flow
app.get("/api/test-resend", async (req, res) => {
  try {
    ...
    const data: any = await response.json();
    console.log("Resend API test email response:", data);

    if (!response.ok) {
      return res.status(response.status).json({ success: false, error: data.message || "Failed to dispatch test mail" });
    }
