...
export default function HowItWorks() {
  const steps = [
    ...
    },
    {
      ...
      icon: <HeartHandshake className="w-5 h-5 text-brand-cyan" />,
    },
  ];

  return (
    <section id="how-it-works" className="py-20 bg-slate-900/20 border-y border-slate-800/50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        ...
        <div className="max-w-2xl mx-auto mb-16">
          <span className="text-xs uppercase tracking-[0.15em] text-indigo-400 font-bold block mb-3 animate-pulse">
            Precision System
          </span>
        ...
        {/* Steps Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 relative items-stretch max-w-6xl mx-auto">
          {steps.map((step, index) => (
            <div
              key={index}
              className="flex flex-col bg-slate-800/40 border border-white/10 rounded-2xl p-6 text-left relative transition-all duration-300 hover:border-brand-cyan/20 group hover:-translate-y-1 shadow-md hover:shadow-black/20"
            >
              
              {/* Connector line on larger screens */}
              {index < 3 && (
                <div className="hidden lg:block absolute top-12 left-[calc(100%_-_1.5rem)] w-12 h-[1px] bg-gradient-to-r from-brand-cyan/30 to-transparent z-0" />
              )}

              {/* Number and icon badge */}
              <div className="flex items-center justify-between mb-5">
                <span className="font-display font-extrabold text-white/10 text-4xl leading-none">
                  {step.num}
                </span>
                <div className="w-9 h-9 rounded-lg bg-brand-cyan/5 border border-brand-cyan/10 flex items-center justify-center group-hover:border-brand-cyan/30 group-hover:bg-brand-cyan/10 transition-colors duration-300">
                  {step.icon}
                </div>
              </div>

              {/* Text description */}
              <h3 className="font-display font-bold text-sm sm:text-base text-white tracking-tight mb-2 group-hover:text-brand-cyan transition-colors duration-300">
                {step.title}
              </h3>
